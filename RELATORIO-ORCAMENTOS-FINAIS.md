# Relatório — Orçamentos finais Valvic (consolidado)

**Gerado por:** Lavinia · **Data:** 16/07/2026 · **Branch:** `claude/fervent-maxwell-55vwy1`

## Certificação de backup
- **Git:** ✅ **Tudo commitado e enviado (push) para o `origin`.** Working tree limpo, branch sincronizada. Esta é a **fonte durável** — nada se perde.
- **Google Drive:** 🟡 **Leitura OK, escrita BLOQUEADA nesta sessão** (cada gravação pede aprovação interativa, que não existe no modo remoto). **Não consegui subir/atualizar arquivos no Drive** — abaixo marco o que já está lá e o que precisa ser subido manualmente (ou liberando a escrita do conector).

---

## 1) Orçamentos desta sessão

| Cliente | Escopo | Configuração final | Status | Drive |
|---|---|---|---|---|
| **Jairo Samuel** | Apto completo — 9 móveis MDF (cristaleira, cozinha, cabeceira, 2 roupeiros, painéis). Parceria **Jéssica Sollero** (RT 10%) | **2 linhas:** Essencial **R$ 70.400** (Hardt/5 anos) · Essencial Prime **R$ 83.900** (Hettich/10 anos, engenharia alemã) | 🟢 Proposta entregue **v003** (capa+galeria com renders do cliente) | ❌ **não está no Drive** |
| **Camila** | Closet — roupeiro em L (piso-teto) + ilha | Config única **R$ 24.900** (ilha virou marcenaria/4 gavetas R$ 4.200; pagto 50/50 **−8% → R$ 22.908**) | 🟢 Concluído (rev. 10/07) | ⚠️ **Docs desatualizados** (versão 27/06); atualizar p/ R$ 24.900 |
| **Maria** | Lavanderia + complemento de painel (Vale dos Cristais) | **R$ 7.850** (derivado da cabeceira-couro, valores mantidos) | 🟢 Entregue | ⚠️ só versões antigas no Drive |
| **Lilian Lee** | Apto completo — marcenaria + portas provençais. Decoradora **Jéssica Sollero** (RT 10%) | **3 linhas:** Essencial **R$ 127.000** (2a) · Conforto **R$ 137.000** (5a) · Premium **R$ 180.500** (10a) + logística Vila Velha à parte | 🟢 Concluído | ❌ **não está no Drive** |
| **Hotel Ibis Betim** | Hall comercial — nichos + molduras + balcão (interior MDF branco) | **R$ 16.500** (MC 31%, RT 10% DAD, pagto 50/50 sem cartão) | 🟢 Concluído | ⚠️ pasta `hotel_ibis` existe, **proposta não subida** |
| **Resolve Consórcio** | Comercial — 14 ambientes (recepção, salas, 63 lockers, 7 cabines acústicas). Parceria Jéssica | **Refeito peça-a-peça** (79 chapas + ripado Eucatex real): Essencial **R$ 147.800** (Hardt/5a) · Prime **R$ 174.800** (Hettich/10a). Pagto 40/40/20, prazo 90–100d | 🟢 **Entregue** (formato Jairo, renders do cliente) | ❌ **não está no Drive** |
| **SPE Nova Lima 1** (LM) | Stand comercial (2 pranchas exec.) — painéis MDF Arauco Cravo Trend (~107 m²), **pérgola 28 ripas metalon #10×5**, armário gourmet + móvel lounge (Moscada), stand corretores + pilar. Projeto **arq. Lodi Motta/JBA** | **Config única R$ 88.200** (chapa R$500 · sem RT · MC 40%; veneziana e vidro jateado = terceiros, fora). Pagto 40/40/20, prazo 45–60 úteis | 🟢 Proposta entregue (comercial, 3 págs) | ❌ **não está no Drive** |
| **Graça** | 3 ambientes — Despensa (armário revestido + bancada L) · Lavanderia (torre máq. lavar) · Armário-tanque. **Projeto 100% MDF Azul Petróleo Guararapes**, tampos em pedra (marmorista). Projeto **arq. Lais Teles** (RT 10%) | **Cálculo peça-a-peça** (9 chapas, nesting unificado): Essencial **R$ 20.100** (telescópica/2a · MC 32%) · Essencial Prime **R$ 24.000** (oculta alemã/5a · MC 37%). Pagto padrão c/ escala, prazo 60–90d | 🟢 Proposta entregue (3 págs, layout leve) | ❌ **não está no Drive** |

## 2) Orçamentos anteriores (no repositório)

| Cliente | Escopo | Configuração final | Status | Drive |
|---|---|---|---|---|
| **Raquel** | Roupeiro Branco TX 6,30 m (piso-teto) | Premium **R$ 30.000** (Hardt/5a) · Essencial **R$ 25.000** (2a) | 🟢 Concluído | ✅ Doc na pasta "Raquel" |
| **Ed. Luxemburgo** | Halls (revestimento MDF amadeirado + espelhos) | Unitário: 2 apt **R$ 9.070** · 4 apt **R$ 12.710** · social **R$ 4.780** · espelhos **R$ 8.730** (MC 37%) | 🟢 Concluído | ✅ subida (pasta do cliente) |
| **Kênia & Fábio** | Casa completa — 15 ambientes, ~60 módulos (arq. Inédita) | **R$ 481.000** (MC 42%) · proposta premium em repo | 🟢 Orçado / proposta pronta | 84 pranchas no Drive (proposta a confirmar) |
| **Karina Ramos** | Escritório / home office (arq. Ionah Pinho) | Proposta Gold/Essencial em repo | 🟢 Proposta pronta | a confirmar |
| **Lucas e Ana** | Apto 101 | **Projeto-exemplo/referência** (pipeline) | 📘 Referência | fontes/ |

---

## 3) O que precisa subir/atualizar no Drive (escrita bloqueada nesta sessão)
Ação manual do Jonathan (arrastar PDF) **ou** liberar a escrita do conector Google Drive:
1. **Jairo Samuel** — criar pasta + subir `proposta-jairo-samuel.pdf` (v003).
2. **Lilian Lee** — criar pasta + subir `proposta-lilian-lee.pdf`.
3. **Camila** — substituir os Docs antigos pela proposta **rev. 10/07** (R$ 24.900). Apagar os 2 Docs de 2-cenários.
4. **Hotel Ibis** — subir `proposta-hotel-ibis.pdf` na pasta `hotel_ibis` (já existe).
5. **Maria** — subir `proposta-maria-lavanderia-painel.pdf` + `aditivo-contratual-maria-lavanderia-painel.pdf` na pasta "Maria".
6. **Resolve Consórcio** — criar pasta + subir `proposta-resolve-consorcio.pdf` (R$ 147.800 / 174.800).
7. **Graça** — criar pasta + subir `proposta-graca.pdf` (R$ 20.100 / 24.000).
8. **SPE Nova Lima 1 (LM)** — criar pasta + subir `proposta-lm.pdf` (R$ 91.300).
> Todos os arquivos acima estão **salvos e versionados no Git** (branch acima) — o Drive é só a cópia de acesso do time.

## 4) Método consolidado — calcular, nunca estimar
Após o episódio do Resolve (estimativa preliminar superestimada em ~R$ 241k/281k), a regra virou princípio nº 1 da skill: **quantitativo sai do plano de corte peça-a-peça** (`corte-*.py`), não de fator de área. Resolve e Graça já nasceram assim (script auditável, chapa a chapa, ripado/pedra/eletro tratados como terceiro quando é o caso). Nada de número que não se possa provar.

**Arquivos-fonte no repositório:** `.claude/skills/orcamentista-marcenaria/projetos/` (todos os `.md`, `.json`, `.html`, `.pdf`) · `MOLESKINE.md` (histórico e aprendizados).
