# Pendências de retificação — dados financeiros e de pessoal

> Levantadas em 08/08/2026 ao cruzar o **extrato do sistema financeiro**
> (`Extrato_08_08_2026.xlsx`) com o **Caderno Empresarial** e as lâminas do Walton.
> Riscar cada item quando corrigido na origem.

---

## A · Corrigir no sistema financeiro

| # | Item | Está no sistema | Deve ser | Impacto |
|---|---|---|---|---|
| A1 | **Salário da Karla** | R$ 1.800/mês (1.000 salário + 800 adiantamento) | **R$ 2.000/mês** | +R$ 200/mês · Jonathan já sinalizou que vai atualizar |
| A2 | **Emive — vigilância** | ago R$ 387,00 · set R$ 280,46 | **R$ 390,00/mês** | +R$ 112,54 nos dois meses |
| A3 | **Pró-labore duplicado** | Setembro tem `pro.lab. Jonathan / adiant.` 2 × R$ 5.000 **mais** `Pro labore Jonathan - Restante` R$ 3.850 | Só os 2 × R$ 5.000 | **R$ 3.850 lançados a mais em setembro.** Resíduo do esquema antigo (7.700 = 3.850 + 3.850). Paulo não tem esse resíduo |
| A4 | **RT de Isabella Biancardine** | Não lançada | Projeto **Simony** · estimada em R$ 1.700 (5%) | Entrou no cálculo do cenário real, mas precisa de lançamento |
| A5 | **Compra de material futura** | Só R$ 5,3k de competência ago em diante | A produção de ago/set exige mais | Jonathan estimou **+R$ 15 mil/mês**, já aplicado no cenário |

## B · Corrigir no Caderno Empresarial

| # | Item | Está no caderno | Deve ser |
|---|---|---|---|
| B1 | **Vínculo do Cezar** | Já consta CLT R$ 2.900 + VR 400 + VT 400 | **Confirmar** — bate com o sistema (2.000 salário + 900 adiantamento + 800 benefícios = R$ 3.700 de custo total). Verificar se algum outro documento ainda o traz como PJ |
| B2 | **Salário da Karla** | R$ 2.000 | Correto — a divergência está no sistema (item A1) |
| B3 | **Custo mensal da estrutura** | R$ 55,2k hoje → R$ 62,9k completo (só folha) | O número real de **custo fixo** é **folha + estrutura**: R$ 75,1k em agosto e **R$ 77,2 mil em setembro** |
| B4 | **Página nova — Custo Fixo da Estrutura** | Não existe | **Criar** na Parte III, com as duas tabelas do relatório de cenário real: folha por pessoa e estrutura por rubrica |
| B5 | **Jonathan Godoy** | R$ 1.620 (salário mínimo) | Custo total é **R$ 2.421** com benefícios (1.621 + 400 VT + 400 VR) |
| B6 | **Benefícios do Joelson, Jomar e Davi** | VR 350 + VT 400 = R$ 750 | Sistema mostra **VR 400 + VT 400 = R$ 800** |

## C · Divergências entre documentos — decidir a fonte de verdade

| # | Dado | Panorama / rascunho | Sistema / histórico | Observação |
|---|---|---|---|---|
| C1 | **Maria · Vale dos Cristais** | R$ 124.000 (rascunho da reunião) | **R$ 117.738** | O sistema e o histórico concordam. O 124k do rascunho está errado |
| C2 | **André / Alphaville** | R$ 87.000 (panorama) | R$ 70.800 (histórico) | Valor a confirmar. Marcado no histórico como **recebível de investimento dos sócios**, não cobrança de cliente externo |
| C3 | **Cristiane** | R$ 94.000 (panorama) | R$ 101.700 com R$ 79.200 pendente (histórico) | Sistema traz a receber R$ 52.449 |
| C4 | **Lucas Mello** | a receber R$ 22.000 | R$ 24.000 no sistema | Diferença de R$ 2.000 |
| C5 | **Pedido nº 74 — R$ 63.000** | Não consta em nenhum documento comercial | R$ 25.000 recebidos em ago + R$ 38.000 em set | **Contrato ausente do panorama comercial** |
| C6 | **Passivo trabalhista R$ 28.691** | Tratado como "a pagar sem data" | Não aparece no sistema financeiro | Confirmar se já foi quitado na homologação (rescisões de 30/04 e 31/05) |

## C-bis · Completude de setembro no sistema

Verificação feita em 08/08 comparando as duas colunas do extrato:

- **Comissões de obra:** 12 lançamentos em agosto (R$ 5.580) e **zero em setembro**. No cenário real isso
  foi coberto pela estimativa de 6% sobre o valor a entregar, mas o sistema precisa receber os lançamentos.
- **Material:** 22 notas em agosto (R$ 45,6k) contra **13 em setembro** (R$ 21,2k). Parte da diferença é real
  (material já comprado), parte é lançamento que ainda não aconteceu.
- **Itens eventuais só em agosto:** manutenção predial (480), manutenção de equipamentos (375), manutenções
  (280), transporte urbano (250), carretos (200), serviços extras do Joelson (1.500) e uma semana a menos de
  corte de chapas (250) — **R$ 3.335 sem equivalente em setembro**.
- **Cobertura de datas:** agosto de 03 a 30, setembro de 02 a 30 — os dois meses estão completos em calendário.

> Conclusão: o resultado de setembro (+R$ 69,5 mil) tende a **cair** conforme os lançamentos entrarem.
> A margem até zerar os dois meses é de R$ 120,5 mil, então o sinal do resultado não muda — mas o valor sim.

## D · Já resolvido

- ~~Pró-labore R$ 7.700 vs R$ 10.000~~ — o sistema já está em **R$ 10.000 por sócio** (2 × R$ 5.000/mês). O arquivo `custo-fixo.json` do Rodrigo ainda tem R$ 6.500 + ajuda de custo e precisa ser atualizado.
- ~~Bruna, Cezar e Hugo ausentes do financeiro~~ — os três já estão lançados no extrato de 08/08.

---

## Números de referência fechados em 08/08/2026

| Indicador | Agosto | Setembro | Dois meses |
|---|---|---|---|
| Entradas previstas | 231.817 | 232.209 | **464.026** |
| Folha de pagamento | 57.786 | 62.586 | 120.372 |
| Custo fixo da estrutura | 17.280 | 14.580 | 31.861 |
| Material de produção | 64.898 | 36.463 | 101.361 |
| Comissões de obra (est. 6%) | 21.624 | 19.724 | 41.348 |
| RT de parceiros | 6.343 | 16.473 | 22.815 |
| Máquinas, dívida e veículos | 12.907 | 12.907 | 25.814 |
| **Total de saídas** | 180.838 | 162.733 | **343.571** |
| **Resultado de caixa** | **+50.979** | **+69.476** | **+120.455** |

**Custo fixo total (folha + estrutura):** R$ 75.066 em agosto · **R$ 77.166 em setembro**
**Valor a entregar (base de comissão):** ago R$ 360.400 · set R$ 328.738 · total **R$ 689.138**
