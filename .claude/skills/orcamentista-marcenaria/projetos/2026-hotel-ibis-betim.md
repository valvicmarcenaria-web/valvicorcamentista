# Hotel Ibis Betim — Reforma do Hall (escopo marcenaria Valvic)

**Cliente:** Hotel Ibis Betim (Accor) · **Arquiteta:** Marina Dolabella Dubal — DAD (Dubal Arq. & Design)
**Obra:** Betim/MG · **Projeto:** executivo "análise e orçamento" (R00, 12/06/2026) · comercial
**Fechamento (v007):** MC **31%** · **RT 10% (DAD)** · **sem cartão** · **R$ 16.500** — Nichos+molduras **R$ 6.000** · Balcão **R$ 10.500** · Hettich → 10 anos
**Balcão — interior em MDF branco:** só as **faces aparentes + portas** em Acetinatta (Preto/Graffite); **caixaria/prateleiras internas em MDF branco**. ⚠️ **Desvio da spec** da arquiteta (que pediu caixaria toda em Acetinatta Preto) — **registrado na proposta** (descrição + selo) para validar com a DAD. Economia: Acetinatta 3→2 chapas · material R$ 8.044 → R$ 7.264 · total R$ 18.500 → R$ 16.500.
**Pagamento:** **50% entrada + 50% na entrega** (transferência) — sem cartão. Eliminar o parcelamento tirou o custo de 8% do modelo → grande parte da queda R$ 23.000 → R$ 18.500 (o resto veio da MC 33%→31%).
**Revisão de material:** nichos prof. **20 cm** · corrigidos **fita** (era R$ 1.800 → ~R$ 700) e removida a linha avulsa de **meia-esquadria** (R$ 1.500 — já na operação/MC). Balcão reavaliado: **~4,4 m, ~7 portas → 3 chapas Acetinatta** (premium, era 2). Material R$ 9.610 → **R$ 8.044**. Balcão passa a ser o item mais caro (Acetinatta premium + Hettich).
**Fonte:** pasta Drive "hotel_ibis" (EXp.5 planta detalhamento · EXp.6/7 elevações · MD apresentação).

## Escopo VALVIC (só a marcenaria MDF)
| Item | Material | Obs |
|---|---|---|
| **① Nichos** (~12) | MDF Duratex **Carvalho Hanover**, meia esquadria | prof. 20/30 cm · parede E2 do Hall (15,12 m), intercalados com serralheria |
| **② Molduras de sinalização** (3 placas) | Carvalho Hanover, meia esquadria | fixadas sobre a chapa metálica (Sinalização 01/02/03) |
| **③ Buffet** (recepção, ~4,48 m) | caixaria **Acetinatta Preto** + portas de giro **Acetinatta Graffite** | puxador cava, rodapé recuado · **NOVO** (não reaproveitar) — deixar **explícito na proposta**: peça nova em **material premium** |

## FORA do escopo Valvic (outros fornecedores)
Serralheria (pilares metálicos, painéis de chapa perfurada 12×25mm, cestos metálicos, nicho metálico) ·
estofados (bancos espuma D33 + tecido Quacker) · placas de sinalização em adesivo · portas/painéis de
vidro · banco em alvenaria · **2 unidades "MARCENARIA EXISTENTE"** (mantidas).

## Quantitativo + custo (1ª passada)
- **Carvalho Hanover:** ~15 m² (12 nichos + 3 molduras) × fator meia-esquadria 1,25 → **5 chapas** (R$ 500).
- **Acetinatta** (buffet): **2 chapas** (1 Preto caixaria + 1 Graffite portas) — premium (~R$ 1.300).
- Chapas R$ 5.100 + fita 1.800 + **mão de obra meia-esquadria 1.500** + ferragens (Häfele + cava) 460 + consumíveis 600 = **material R$ 9.460**.
- Terceirizados **R$ 0** (serralheria/estofado/vidro são de outros) · visitas R$ 250 → **fixedR R$ 9.710**.
- Ferragem **Hettich** (garantia 10 anos): ferragens R$ 610 → material R$ 9.610 · fixedR R$ 9.860.
- `inv = fixedR / (1 − 0,18 − 0,88·0,143 − MC)` com **RT 10%** → **MC 33% = R$ 27.000**. **RT p/ a DAD ~R$ 2.376.**
- **Investimento separado (proporcional ao material):** Nichos + molduras **R$ 14.400** · **Balcão** novo (Acetinatta) **R$ 12.600**.
- Proposta: 4 páginas (removida a pág. "O projeto da DAD"); balcão em linha separada; buffet marcado como NOVO + premium.

## Notas técnicas da arquiteta (impactam custo)
Ferragens **FGV ou Häfele** c/ amortecedor · **todas as bordas fitadas** (mesmo ocultas) · marcenaria
**modular** (sem içamento) · **conferir medidas no local** · **validar amostras** com a arquiteta ·
tudo em **meia esquadria** (junção 45° = acabamento premium, mão de obra alta → CX alto).

## Flags / a confirmar (antes de fechar)
1. **Contagem/tamanho dos nichos** (usei ~12 · confirmar nicho a nicho nas elevações E1–E4).
2. **Buffet: NOVO ou reface do existente?** — a arquiteta pede "verificar aproveitar" → muda o Acetinatta (premium).
3. **MC alvo** — usei **42%** (comercial); Rodrigo define o piso (projeto pequeno + CX alto → não descer de 40%).
4. **RT** — não há RT de arquiteta definido (usei 0); confirmar se a DAD entra com RT.
5. Confirmar se **fundos dos nichos** são Carvalho Hanover (aparentes) ou revestidos internos.

**Arquivos:** `orcamento-hotel-ibis-betim.json` · este `.md`. Pranchas-fonte anexadas na sessão.
